...
AkhmetovDStrassenDenseDoubleALL::AkhmetovDStrassenDenseDoubleALL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool AkhmetovDStrassenDenseDoubleALL::ValidationImpl() {
...
bool AkhmetovDStrassenDenseDoubleALL::PreProcessingImpl() {
  const auto &input = GetInput();
  const size_t n = format::GetN(input);
...
bool AkhmetovDStrassenDenseDoubleALL::RunImpl() {
  const auto &input = GetInput();
...
#pragma omp parallel default(none) shared(counter) num_threads(ppc::util::GetNumThreads())
...

  {
    ...
    for (int i = 0; i < num_threads; i++) {
      threads.at(i) = std::thread([&]() { counter++; });
      threads.at(i).join();
    }
  }
...
bool AkhmetovDStrassenDenseDoubleALL::PostProcessingImpl() {
  const auto &input = GetInput();
  const size_t n = format::GetN(input);
